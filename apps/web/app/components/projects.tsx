				...
				<div key={project.id} className="space-y-2 mb-8">
					<h2 className="text-lg font-semibold">
						<div className="flex gap-2 items-center">
							<Link className="flex gap-2 items-center" href={`/projects/${project.id}`}>
						...
						{project.config &&
							...
							project.config.services.map((service, index) => (
								<Link href={`/projects/${project.name}/app/${service.data.serviceName}`} key={index}>
									<li key={index} className="border p-4 rounded shadow text-sidebar-foreground bg-background">
										<div className="truncate w-full font-semibold">{service.data.serviceName}</div>
										<div className="text-xs text-gray-600">{service.data.source.image}</div>
										<div className="text-sm mt-2">
											{service.data.domains.map((domain: any, i: number) => (
												<div key={i}>
													<span className="text-muted-foreground">{domain.host}</span>: {domain.port}
												</div>
											))}
										</div>
									</li>
