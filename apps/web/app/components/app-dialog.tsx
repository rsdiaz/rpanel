    ...
    <div className="">
      ...
      >
        <div>
          {Object.keys(schema.properties).map((key) => {
            ...
            return (
              <div key={key} className="flex flex-col gap-2 mb-4">
                <Label htmlFor={key}>{property.title || key} {schema.required.includes(key) ? <span className="text-red-400">*</span> : ''}</Label>
                <Input
                  type="text"
                  id={key}
                  name={key}
          ...
          <Button variant={"secondary"} className="bg-blue-600 hover:bg-blue-700 text-white">Create</Button>
        </div>
      </form>
    </div>
  );
              ...
              <div className="flex gap-4">
                {appTemplate?.meta.links.map((info) => (
                  <Link key={info.label} href={info.url} target="_blank">
                    <Badge className="bg-background-secondary text-muted-foreground hover:bg-background-secondary hover:text-foreground transition-colors duration-300">
                      {info.label}
                    </Badge>
                  </Link>
                ))}
